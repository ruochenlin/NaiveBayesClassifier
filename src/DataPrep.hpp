...
void dataPrep(string const &trainFile, string const &testFile)
{
	...
	fout << "#!/bin/bash" << endl <<
		...
		"grep attribute " << newTrainFile << "  | sed -e 's/{/ nominal /g' | sed -e 's/numeric/real/g' | sed -e 's/}//g' | sed -e \"s/' / /g\" | awk -F \"'\" '{$1=\"\"; print $0}' | sed -e 's/,/ /g' | sed -e 's/  */ /g' | sed -e 's/\r//g'>> attr.tmp " << endl <<
		...
		"grep attribute " << newTestFile << " | sed -e 's/{/ nominal /g' | sed -e 's/numeric/real/g' | sed -e 's/}//g' | sed -e \"s/' / /g\" | awk -F \"'\" '{$1=\"\"; print $0}' | sed -e 's/,/ /g' | sed -e 's/  */ /g' | sed -e 's/\r//g'>> attr2.tmp " << endl <<
